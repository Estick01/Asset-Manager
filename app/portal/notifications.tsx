...
const TIPO_CONFIG: Record<string, { icon: keyof typeof Ionicons.glyphMap; color: string }> = {
  nueva_actualizacion: { icon: "megaphone-outline",          color: Colors.primary },
  nuevo_documento:     { icon: "document-attach-outline",    color: Colors.info },
  estado_cambio:       { icon: "refresh-circle-outline",     color: Colors.warning },
  default:             { icon: "notifications-outline",      color: Colors.textSecondary },
};
...
export default function ClientNotificationsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getNotificacionesCliente(profile.id);
  ...
  const handleMarkRead = async (item: Notificacion) => {
    if (!item.leidoCliente) {
      ...
      setNotificaciones((prev) =>
        prev.map((n) => (n.id === item.id ? { ...n, leidoCliente: true } : n))
      );
      refreshUnread();
    }
    if (item.procesoId) {
      router.push({ pathname: "/portal/case", params: { id: item.procesoId } });
    }
  };

  const handleMarkAll = async () => {
    if (!profile?.id) return;
    await markTodasLeidasCliente(profile.id).catch(() => {});
    setNotificaciones((prev) => prev.map((n) => ({ ...n, leidoCliente: true })));
    refreshUnread();
  };

  const unread = notificaciones.filter((n) => !n.leidoCliente).length;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {loading ? (
        <ActivityIndicator style={{ marginTop: 40 }} color="#1B5A8C" />
      ) : (
        <FlatList
          data={notificaciones}
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                style={[styles.card, !item.leidoCliente && styles.cardUnread]}
              ...
              >
                ...
                {!item.leidoCliente && <View style={[styles.dot, { backgroundColor: "#1B5A8C" }]} />}
              </Pressable>
            );
          }}
        />
...
const styles = StyleSheet.create({
  ...
  markAllBtn: {
    ...
    paddingVertical: 6,
    backgroundColor: "#1B5A8C18",
    borderRadius: 8,
  },
  markAllText: { fontSize: 13, fontFamily: "Inter_500Medium", color: "#1B5A8C" },
  ...
  cardUnread: { borderLeftWidth: 3, borderLeftColor: "#1B5A8C" },
