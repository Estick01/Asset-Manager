...
export default function FirmNotificationsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await getNotificacionesFirma(profile.id);
  ...
  const handleMarkRead = async (item: Notificacion) => {
    if (!item.leidoFirma) {
      ...
      setNotificaciones((prev) =>
        prev.map((n) => (n.id === item.id ? { ...n, leidoFirma: true } : n))
      );
      refreshUnread();
    }
    if (item.procesoId) {
  ...
  const handleMarkAll = async () => {
    if (!profile?.id) return;
    await markTodasLeidasFirma(profile.id).catch(() => {});
    setNotificaciones((prev) => prev.map((n) => ({ ...n, leidoFirma: true })));
    refreshUnread();
  };

  const unread = notificaciones.filter((n) => !n.leidoFirma).length;
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                style={[styles.card, !item.leidoFirma && styles.cardUnread]}
              ...
              >
                ...
                {!item.leidoFirma && <View style={styles.dot} />}
              </Pressable>
            );
          }}
        />
