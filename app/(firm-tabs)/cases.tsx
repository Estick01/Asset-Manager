...
import { type ProcesoDTO, type EstadoProceso } from "@/shared/schema";
...
export default function FirmCasesScreen() {
  ...
  const fetchProcesos = useCallback(async (searchTerm?: string, estadoCodigo?: string | null, responsableFilter?: boolean | null, isLoadMore = false) => {
    try {
      const currentOffset = isLoadMore ? offsetRef.current : 0;
      const result = await getProcesos(PAGE_SIZE, currentOffset, {
        search: searchTerm || undefined,
        estadoCodigo: estadoCodigo || undefined,
        hasResponsable: responsableFilter !== null && responsableFilter !== undefined ? responsableFilter : undefined,
      });

      if (isLoadMore) {
        setProcesos(prev => [...prev, ...(result.data as ProcesoDTO[])]);
      } else {
        setProcesos(result.data as ProcesoDTO[]);
      }
  ...
  const onRefresh = useCallback(async () => {
    offsetRef.current = 0;
    setRefreshing(true);
    await fetchProcesos(search, selectedEstado, hasResponsable);
    setRefreshing(false);
  }, [search, selectedEstado, hasResponsable, fetchProcesos]);

  const handleLoadMore = useCallback(() => {
    if (loadingMore || procesos.length >= total) return;
    setLoadingMore(true);
    fetchProcesos(search, selectedEstado, hasResponsable, true);
  }, [loadingMore, procesos.length, total, search, selectedEstado, hasResponsable, fetchProcesos]);
  ...
  const handleSearch = (text: string) => {
    ...
    searchTimer.current = setTimeout(() => {
      setLoading(true);
      fetchProcesos(text, selectedEstado, hasResponsable);
    }, 400);
  };
  ...
  const handleEstadoFilter = (codigo: string | null) => {
    ...
    offsetRef.current = 0;
    setLoading(true);
    fetchProcesos(search, codigo, hasResponsable);
  };
  ...
  const handleResponsableFilter = (value: boolean | null) => {
    ...
    offsetRef.current = 0;
    setLoading(true);
    fetchProcesos(search, selectedEstado, value);
  };
      ...
      <View style={styles.body}>
        ...
        <View style={styles.chipsWrap}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
            ...
            >
              ...
              <Text style={[styles.chipText, hasResponsable === null && styles.chipTextActive]}>
                Todos
              </Text>
            </Pressable>
            <Pressable
        ...
        {loading ? (
          ...
          <>
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <ProcesoCard
                  item={item}
                  index={index}
                  pageOffset={procesos.length - (procesos.length % PAGE_SIZE || PAGE_SIZE)}
                />
              )}
              contentContainerStyle={{ paddingHorizontal: 16, gap: 10, paddingTop: 4 }}
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptyTitle}>
                    {search ? "Sin resultados" : "Sin procesos"}
                  </Text>
                  <Text style={styles.emptySub}>
                    {search
                      ? `No se encontraron procesos para "${search}"`
            ...
            {/* ✅ Footer FUERA del FlatList — siempre visible en web y mobile */}
            {loadingMore && (
