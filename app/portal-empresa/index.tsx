...
export default function PortalEmpresaScreen() {
  ...
  const empresa = (profile as any)?.empresa;
  const razonSocial = empresa?.razonSocial ?? user?.user?.name ?? "Empresa";
  ...
  const activos     = procesos.filter(p => ["activo", "en_tramite"].includes(p.estado?.codigo ?? "")).length;
  ...
  const renderProceso = ({ item: p }: { item: ProcesoDTO }) => {
    ...
    return (
      <Pressable
        ...
        onPress={() => router.push({ pathname: "/portal-empresa/case", params: { id: p.id } })}
      >
        ...
        <View style={styles.cardBody}>
          <View style={styles.cardHeader}>
            <View style={styles.cardTitleSection}>
              ...
              {p.tipoProceso?.nombre && <Text style={styles.tipo}>{p.tipoProceso.nombre}</Text>}
            </View>
          ...
          <View style={styles.cardFooter}>
            ...
            <View style={styles.metaRow}>
              ...
              <Text style={styles.metaText}>
                {new Date(p.fechaCreacion).toLocaleDateString("es-CO", { day: "numeric", month: "short", year: "numeric" })}
              </Text>
            </View>
          </View>
        </View>
  ...
  const ListHeader = () => (
    <>
        ...
        <View style={styles.headerTop}>
          <View style={{ flex: 1 }}>
            <View style={styles.empresaBadge}>
              ...
              <Text style={styles.empresaLabel}>Portal Empresa</Text>
            </View>
            <Text style={styles.clientName} numberOfLines={2}>{razonSocial}</Text>
          </View>
      ...
      <View style={styles.listHeader}>
        ...
        {procesos.length > 0 && <Text style={styles.sectionCount}>{procesos.length}</Text>}
      </View>
    </>
  );
...
const styles = StyleSheet.create({
  ...
  empresaBadge: { flexDirection: "row", alignItems: "center", gap: 5, marginBottom: 4 },
  empresaLabel: { fontSize: 12, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.7)" },
  clientName: { fontSize: 20, fontFamily: "Inter_700Bold", color: Colors.white, marginTop: 2, flex: 1 },
  headerBtn: {
  ...
  sectionCount: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: Colors.textTertiary },
  card: {
    flexDirection: "row", alignItems: "center",
    backgroundColor: Colors.white, borderRadius: 16, overflow: "hidden", marginHorizontal: 16,
  ...
  tipo: { fontSize: 12, fontFamily: "Inter_400Regular", color: Colors.textSecondary, marginTop: 2 },
  estadoBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9, paddingVertical: 4, borderRadius: 10 },
  estadoDot: { width: 6, height: 6, borderRadius: 3 },
  estadoText: { fontSize: 11, fontFamily: "Inter_600SemiBold" },
  cardFooter: { flexDirection: "row", gap: 14 },
  metaRow: { flexDirection: "row", alignItems: "center", gap: 4 },
  metaText: { fontSize: 11, fontFamily: "Inter_400Regular", color: Colors.textTertiary, maxWidth: 140 },
  emptyState: { alignItems: "center", justifyContent: "center", gap: 10, paddingVertical: 60, paddingHorizontal: 32 },
  emptyIconWrap: {
  ...
  emptySubtitle: { fontSize: 13, fontFamily: "Inter_400Regular", color: Colors.textTertiary, textAlign: "center" },
